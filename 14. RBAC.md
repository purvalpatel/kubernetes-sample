...
--------------------------------------------
- We have one namespace (numol) and want to provide Edit access of that namespace to developer.

**Below are the steps that we need to follow.** <br>
Lets assume: <br>
**username** - numol_read <br>
**namespace** - numol <br>

## STEP 1 : Create namespace ( if dont have any ):
```
create namespace numol
```

## STEP 2 : Create Certificate for the user:
### Create Private Key.
```
openssl genrsa -out numol_read.key 2048
```

### Create CSR:
```
openssl req -new -key numol_read.key -out numol_read.csr -subj "/CN=numol_read/O=developers"
```

Here, <br>
CN = username (numol_read) <br>
...
Create YAML. <br>
Generate `numol_read-csr.YAML` <br>
...
spec:
  ...
  request: $(cat numol_read.csr | base64 | tr -d '\n')
  signerName: kubernetes.io/kube-apiserver-client
  usages:
  - client auth" > numol_read-csr.yaml
```
### Apply CSR: <br>
```
kubectl apply -f numol_read-csr.yaml
```
...
```
kubectl get csr numol_read -o jsonpath='{.status.certificate}' | base64 -d > numol_read.crt
```
Now you have: <br>
**numol_read.key** <br>
**numol_read.crt** <br>
...
## Create user kubeconfig ( `numolread.kubeconfig`)
numolread.kubeconfig
```
apiVersion: v1
kind: Config
clusters:
...
- name: numol_read
  user:
    client-certificate: numol_read.crt
    client-key: numol_read.key
